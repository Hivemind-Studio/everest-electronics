...
- Name: site.spec.ts >> Home page — Figma design sections >> renders Clients section with brand list
- Location: tests-e2e/site.spec.ts:47:7
...
Locator: getByText('PANASONIC')
Expected: visible
Error: strict mode violation: getByText('PANASONIC') resolved to 2 elements:
    1) <p class="mt-2 text-sm text-graphite">Awarded by Panasonic Indonesia</p> aka getByText('Awarded by Panasonic Indonesia')
    2) <span class="font-display text-xl font-bold uppercase tracking-wide text-mist transition-colors hover:text-ink">Panasonic</span> aka getByText('Panasonic', { exact: true })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('PANASONIC')
