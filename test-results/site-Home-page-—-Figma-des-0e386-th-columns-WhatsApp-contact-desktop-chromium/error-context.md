...
- Name: site.spec.ts >> Home page — Figma design sections >> renders Footer with columns + WhatsApp contact
- Location: tests-e2e/site.spec.ts:79:7
...
Locator: getByText('LAYANAN')
Expected: visible
Error: strict mode violation: getByText('LAYANAN') resolved to 12 elements:
    1) <a href="/#layanan" aria-current="page" class="text-sm font-medium transition-colors text-navy">Layanan</a> aka getByRole('link', { name: 'Layanan', exact: true })
    2) <a href="#layanan" class="btn-outline bg-white/5 px-7 py-3.5 text-sm text-white border-white/25 hover:border-white">Jelajahi Layanan</a> aka getByRole('link', { name: 'Jelajahi Layanan' })
    3) <p class="mt-8 max-w-lg font-display text-2xl font-medium leading-snug text-ink-soft">“Visi kami adalah menjadi rekanan utama dalam pen…</p> aka getByText('“Visi kami adalah menjadi')
    4) <h3 class="font-display text-xl font-bold text-ink">Kualitas Layanan yang Konsisten</h3> aka getByRole('heading', { name: 'Kualitas Layanan yang' })
    5) <p class="mt-2 text-base leading-relaxed text-graphite">Menyediakan layanan dengan standard dan kualitas …</p> aka locator('#tentang').getByText('Menyediakan layanan dengan')
    6) <h3 class="font-display text-xl font-bold text-ink">Inovasi dalam Layanan</h3> aka getByRole('heading', { name: 'Inovasi dalam Layanan' })
    7) <p class="mt-2 text-base leading-relaxed text-graphite">Mengadopsi teknologi terbaru dan metode inovatif …</p> aka getByText('Mengadopsi teknologi terbaru')
    8) <p class="max-w-sm text-base text-white/70">Menyediakan layanan dengan standard dan kualitas …</p> aka locator('#penghargaan').getByText('Menyediakan layanan dengan')
    9) <p class="mt-2 text-sm text-graphite">Kategori Layanan & Penjualan Terbaik</p> aka getByText('Kategori Layanan & Penjualan')
    10) <p class="mt-2 line-clamp-2 text-sm text-graphite">Komitmen kami untuk selalu memberikan layanan pur…</p> aka getByRole('link', { name: 'Pentingnya Menjaga Kualitas' })
    ...

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('LAYANAN')
